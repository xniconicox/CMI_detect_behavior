...
class MultimodalTrainerV31:
    """多モダリティモデル学習管理クラス（v31）"""
    ...
    def train_cross_validation(
        ...
        for fold, (tr_idx, val_idx) in enumerate(skf.split(np.arange(len(y)), y, groups), 1):
            ...
            # モデル保存
            model_path = self.model_dir / f"multimodal_model_v31_fold{fold}.keras"
    ...
    def plot_training_curves(self, fold_histories: list | None = None, save_path: str | Path | None = None) -> None:
        ...
        fig.suptitle('Training Curves (Multimodal Model V31)', fontsize=16)
