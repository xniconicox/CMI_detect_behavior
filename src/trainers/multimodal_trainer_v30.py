...
class MultimodalTrainer:
    ...
    def _train_fold(
        ...
        *,
        epochs: int = 50,
        batch_size: int = 32,
    ) -> tuple[keras.Model, keras.callbacks.History, float]:
        ...
        f1 = f1_score(y[val_idx], preds.argmax(axis=1), average="macro")
        return model, history, float(f1)
    ...
    def train(self, data: Dict[str, np.ndarray], epochs: int = 50, batch_size: int = 32) -> keras.callbacks.History:
        ...
        model, history, _ = self._train_fold(
            X_s,
            X_d,
            X_t,
            X_f,
