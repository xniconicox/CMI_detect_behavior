...
```
submissions/ensemble_v31/
├── src/
│   ├── inference_pipeline.py      # 5-foldアンサンブル推論パイプライン
│   ├── trainers/
│   │   └── multimodal_trainer_v31.py  # モデル定義
│   └── utils/                     # 共通ユーティリティ
...
```bash
# プロジェクトルートから
./scripts/run_multimodal_training_v31.sh
```

### 2. ローカルでの推論実行

```bash
cd submissions/ensemble_v31
./run_local.sh
```
...
1. `submissions/ensemble_v31`をzip化
...
# Kaggle Notebook内
import zipfile
zipfile.ZipFile('/kaggle/input/your-uploaded-zip/ensemble_v31.zip').extractall('.')

# ノートブックを実行
exec(open('ensemble_v31/cmi-2025-demo-submission.ipynb').read())
```
...
```bash
# 学習が完了しているか確認
ls -la output/experiments/multimodal_v31/models/
