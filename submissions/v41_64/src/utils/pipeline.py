...
class TabularFeatureBuilder:
    ...
    def build(
        ...
        *,
        ...
        if flag_cols:
            # ★ デバッグログを追加
            logger.info(f"Found flag columns in df: {flag_cols}")
            grouped = {
            ...
            # BUGFIX: 特徴量は値が0でも常に追加する必要があるため、.any()の条件分岐を削除
            # if flag_array.any():
            feats.append(flag_array)

        else:
            # ★ デバッグログを追加
            logger.info("No flag columns found in df. Skipping flag features.")

        # 最終的なNaN値チェック
        # ★ デバッグログを追加
        logger.debug(f"Feature shapes before hstack: {[f.shape for f in feats]}")
        logger.debug(f"X_demo shape: {X_demo.shape}")
...
class Preprocessor:
    ...
    def transform(
        ...
        *,
        ...
        print("DEBUG: tab shape after build", tab.shape)
