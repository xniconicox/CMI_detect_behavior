...
def test_sequence_mode_returns_mask(tmp_path: Path):
    cfg = {
        "cache_dir": str(tmp_path / "cache"),
        "preprocessing": {
            "window_size": 16,
    ...
    df = make_dummy_df(20)
    pp = Preprocessor(cfg, use_interp_cleaning=False)
    out = pp.fit_transform(df, use_windows=False)

    assert "sequences" in out and "sequence_mask" in out
    B, T, D = out["sequences"].shape
    assert out["sequence_mask"].shape == (B, T)
